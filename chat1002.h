...
/* Functions defined in knowledgebase.c */
void strcat_special(char *dest, char *src[], size_t src_size, size_t n, int offset);
IER_Node * MakeNode(const char *entity, const char *resp);
void AddtoLL(IER_Node *head, IER_Node *node);
void FreeLL(IER_Node *node);
int kb_update_what(IER_Node *new_node);
int kb_update_where(IER_Node *new_node);
int kb_update_who(IER_Node *new_node);
